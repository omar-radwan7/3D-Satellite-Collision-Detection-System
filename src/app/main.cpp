...
void mouse_callback(GLFWwindow* window, double xposIn, double yposIn) {
    ...
    if (leftClick) {
        if (shiftPressed) {
            ...
            earth->Rotate(xoffset * 0.5f, -yoffset * 0.5f);
        } 
...
int main() {
    ...
    for(int i = 0; i < 100; ++i) {
        Satellite s;
        s.id = 100 + i;
        s.semiMajorAxis = 7000.0f + (rand() % 5000);
        s.eccentricity = (rand() % 200) / 1000.0f;
        ...
        if (alt < 1000) s.color = glm::vec3(1.0, 0.2, 0.2);
        else if (alt < 10000) s.color = glm::vec3(0.2, 1.0, 0.2);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // Update Earth Rotation (Auto-spin)
            ...
            for(const auto& ev : events) {
                ...
                if(activeCollisions.find(key) == activeCollisions.end()) {
                    ...
                    for(const auto& s : satellites) {
                        if(s.id == ev.sat1_id) { p1 = s.position; f1=true; }
                        if(s.id == ev.sat2_id) { p2 = s.position; f2=true; }
                    }
                    if(f1 && f2) {
                        glm::vec3 mid = (p1 + p2) * 0.5f;
                        debrisSystem->addExplosion(mid);
                    }
                }
            }
        ...
        if (cameraFollow && selectedSatId != -1) {
            ...
            for(const auto& s : satellites) {
                if(s.id == selectedSatId) {
                    glm::vec3 targetPos = s.position * (1.0f / 6371.0f);
        ...
        gui->Render(state, *satSystem, *colMan);
        gui->RenderDrawData();
